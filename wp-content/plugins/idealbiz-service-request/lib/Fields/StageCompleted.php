<?php
namespace WidgiLabs\WP\Plugin\IdealBiz\Service\Request\Fields;

/**
 * Registers the "Active Markets" custom field.
 *
 * @since  1.0.0
 * @author WidgiLabs <dev@widgilabs.com>
 */
class StageCompleted {

	/**
	 * Get field definition.
	 *
	 * @since  1.0.0
	 * @return array
	 */
	public static function get() {
		return array(
			'key'               => 'field_5caddf29417ca',
			'label'             => _x( 'Stage completed template', 'Label', 'idealbiz-service-request' ),
			'name'              => 'stage_completed_template',
			'type'              => 'wysiwyg',
			'instructions'      => __( 'You can use the following placeholders.
%ORDER_LINK% - represent the link to accept and pay for the order.
%STAGE% - represent the completed stage.', 'idealbiz-service-request' ),
			'required'          => 0,
			'conditional_logic' => 0,
			'wrapper'           => array(
				'width' => '',
				'class' => '',
				'id'    => '',
			),
			'default_value'     => __( '%STAGE% completed. %ORDER_LINK%', 'idealbiz-service-request' ),
			'tabs'              => 'all',
			'toolbar'           => 'full',
			'media_upload'      => 1,
			'delay'             => 0,
		);
	}
}
