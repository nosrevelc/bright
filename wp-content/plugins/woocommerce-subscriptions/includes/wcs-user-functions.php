...
/**
 * Gets all the active subscriptions for a user, as specified by $user_id
 *
 * @param int $user_id (optional) The id of the user whose subscriptions you want. Defaults to the currently logged in user.
 * @since 2.0
 */
function wcs_get_users_active_subscriptions( $user_id = 0 ) {
 
	if ( 0 === $user_id || empty( $user_id ) ) {
		$user_id = get_current_user_id();
	}

	$subscriptions = apply_filters( 'wcs_pre_get_users_active_subscriptions', array(), $user_id );
	...
	return apply_filters( 'wcs_get_users_active_subscriptions', $subscriptions, $user_id );
}
