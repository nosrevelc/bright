...
if( !class_exists( 'MUCD_Data' ) ) {
    ...
    class MUCD_Data {
        ...
        public static function db_copy_tables( $from_site_id, $to_site_id ) {
            ...
            $excluded_post_types = implode("', '", array(
                'listing',
                'shop_order',
                'attachment',
                'expert',
            ...
            $attachments_ids = implode("', '",MUCD_Files::get_included_files()['ids']);

            foreach ($from_site_table as $table) {

                $original_table_name = substr( $table, $from_site_prefix_length );

                $table_name = $to_site_prefix . $original_table_name;
                ...
                switch ($original_table_name) {
                    ...
                    case 'fep_attachments':
                    case 'fep_messagemeta':
                    case 'fep_messages':
                    case 'fep_participants':
                        break;
                    
                    case 'comments':
                        self::do_sql_query('INSERT `' . $table_name . '` SELECT * FROM `' . $schema . '`.`' . $table . '` WHERE `comment_type` NOT IN (\'order_note\')');
                        break;

                    case 'postmeta':
                        $posts_table = $from_site_prefix . 'posts';
                        self::do_sql_query('INSERT `' . $table_name . '` SELECT * FROM `' . $schema . '`.`' . $table . '` WHERE `post_id` NOT IN (SELECT ID FROM `' . $schema . '`.`' . $posts_table . '` WHERE `post_type` IN (\''.$excluded_post_types.'\') AND `ID` NOT IN (\''.$attachments_ids.'\'))');
                        break;

                    case 'posts':
                        //self::do_sql_query('INSERT `' . $table_name . '` SELECT * FROM `' . $schema . '`.`' . $table . '` WHERE `ID` NOT IN (SELECT ID FROM `' . $schema . '`.`' . $table . '` WHERE `post_type` IN (\''.$excluded_post_types.'\') AND `ID` NOT IN (\''.$attachments_ids.'\'))');
                        self::do_sql_query('INSERT `' . $table_name . '` SELECT * FROM `' . $schema . '`.`' . $table . '` WHERE `post_type` NOT IN (\''.$excluded_post_types.'\') OR `ID` IN (\''.$attachments_ids.'\')');
                        break;
            ...
            if(is_serialized($row[$field])) {
                ...
                if(is_array($row[$field])) {
                    ...
                    foreach($array_object as $key => $field) {
                        $key = trim($key); // Just added this line.
                        $row[$field]->$key = $field;
                    }
                }
                else {
