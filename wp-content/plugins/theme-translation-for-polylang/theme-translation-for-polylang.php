 ...
 */
function tt_pll_esc_html_filter($safe_text, $text)
{
    $translations = get_translations_for_domain('pll_string');
    $translation = $translations->translate($safe_text);
    if (!empty($translation) && $translation !== $safe_text) {
        return $translation;
    }
...
{
    $translations = get_translations_for_domain('pll_string');
    return $translations->translate_plural($translations->translate($single), $translations->translate($plural), $number);
}

//gettext_with_context
add_filter('gettext_with_context', 'tt_pll_gettext_with_context_filter', 999, 4);
function tt_pll_gettext_with_context_filter($translated, $text, $context, $domain)
{
    $translations = get_translations_for_domain('pll_string');
