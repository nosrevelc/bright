...
trait broadcasting
{
	...
	**/
	public function broadcast_post( $broadcasting_data )
	{
		...
		$langsofpost = $bcd->_POST['post_lang_choice'];

		$this->debug( 'The POST language is: '.$langsofpost);
		...
		foreach( $bcd->blogs as $child_blog_id => $child_blog )
		{
			...
			if ( $need_to_insert_post )
			{
				...
				$this->debug( 'going to test language');
				$this->debug( 'post'.var_dump($temp_post_data));
				$this->debug( 'langs'.print_r(pll_languages_list('slug')));
				...
				$this->debug( 'trying to copy post');	

				if (function_exists('pll_the_languages')) { 
					$this->debug( 'have polylang');	
					$site_langs= pll_languages_list();
					foreach (array_keys($site_langs, 'portugues', true) as $key) {
						unset($site_langs[$key]);
					}
					$this->debug( 'Original Lang <pre>%s</pre> language', $langsofpost );	
					$this->debug( 'Destination site langs <pre>%s</pre> language', $site_langs );	


					unset($bcd->parent_blog_taxonomies['location']);
					unset($bcd->parent_blog_taxonomies['listing_cat']);

					$this->debug( 'testlang<pre>%s</pre>',  $bcd->parent_blog_taxonomies);	
			...
			pll_set_post_language($bcd->new_post( 'ID' ), 'en');
	...
	**/
	public function threewp_broadcast_parse_content( $action )
	{
		...
		if ( $action->broadcasting_data->copied_attachments()->count() > 0 )
			...
			$this->debug( 'No attachments known.' );
