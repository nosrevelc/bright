...
class LRM_API_Manager_MENU {
	...
	public function load_settings() {
		...
		add_settings_field( 'ame_activation_server_alternative', __( 'Use Alternative server in case of connection issues?', LRM_API_Manager()->text_domain ), array( $this, 'wc_am_api_server_field' ), LRM_API_Manager()->ame_activation_tab_key, LRM_API_Manager()->ame_api_key );
	...
	public function wc_am_api_key_status() {
        ...
        if ( !$license_status || !is_array($license_status) ) {
            echo 'Can\'t connect to the API server or wrong response: <pre><code>', $license_status, '</code><pre>';
            return;
        }
	...
	public function wc_am_api_server_field() {
		echo "<input type='checkbox' id='ame_activation_server_alternative' name='" . LRM_API_Manager()->ame_data_key . "[ame_activation_server_alternative]' " . checked(true, LRM_API_Manager()->ame_options['ame_activation_server_alternative'], false) . "/> yes";
	}
	...
	public function validate_options( $input ) {
		...
		$options['ame_activation_server_alternative'] = !empty($input['ame_activation_server_alternative']);
		...
		if ( $_REQUEST['option_page'] != LRM_API_Manager()->ame_deactivate_checkbox ) {
			...
			if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $api_email == '' || $checkbox_status == 'on' || $current_api_key != $api_key  ) {
				...
				$args = array(
					'email' => $api_email,
					'licence_key' => $api_key,
					'alternative_server' => $options['ame_activation_server_alternative'],
                );
				...
				if ( $activate_results['activated'] === false || !$activate_results ) {
					...
					$debug_info[] = isset($activate_results['requested_url']) ? 'Requested Url: <code>' . esc_html($activate_results['requested_url']) . '</code>' : '';
					$debug_info[] = isset($activate_results['request']) ? 'Request debug: <code>' . print_r($activate_results['request'], true) . '</code>' : '';
	...
	public function license_key_status() {
        ...
        $status_json = LRM_API_Manager()->key()->status( $args );
