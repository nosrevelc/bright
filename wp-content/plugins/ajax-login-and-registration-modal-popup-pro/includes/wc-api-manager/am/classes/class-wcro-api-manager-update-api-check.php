...
class LRM_API_Manager_Update_API_Check {
	...
	public function update_check( $transient ) {
		...
		if ( isset( $response ) && is_object( $response ) && $response !== false ) {
            ...
            // Reset activation if it was reset on the server
            if ( !(bool)$response->data->activated && lrm_pro_is_license_active() ) {
                $update = array(
...
//		var_dump($new_ver);
//		var_dump($curr_ver);
//		var_dump(version_compare( $new_ver, $curr_ver, '>' ));
		...
		if ( isset( $new_ver ) && isset( $curr_ver ) ) {
			...
			if ( $response !== false && version_compare( $new_ver, $curr_ver, '>' ) ) {
				...
				if ( $this->plugin_or_theme == 'plugin' ) {
					...
					$transient->response[$this->plugin_name] = $response->data->package;
...
//        var_dump($transient);
	...
	public function plugin_information( $args ) {
		...
		if ( defined('LRM_API_LOG_REQUESTS') ) {
			lrm_log( 'API CALL: ' . $target_url );
		}
		...
		if ( is_wp_error( $request ) || !in_array($request_code, [100, 200]) ) {
			return false;
		}
		...
		//lrm_log( "LRM API query", $response );
...
//		}
		...
		$args = array(
			'request'			=> 'information', // plugininformation
			'plugin_name'		=>	$this->plugin_name,
		...
		//var_dump((array)$response->data->info);
