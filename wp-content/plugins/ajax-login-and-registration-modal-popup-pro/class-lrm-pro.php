...
class LRM_Pro {
    ...
    public function init() {
        ...
        add_action('wp_loaded', array($this, 'wp_init'), 8);
	    ...
	    LRM_Pro_Wordfence_2FA::init();
	    LRM_Pro_iThemes_Security_2FA::init();
    ...
    public function wp_init()
    {
	    ...
	    LRM_Pro_Mailchimp_for_Woocommerce::init();
    ...
    public function assets() {
        ...
        $script_params = array(
            ...
            'buddypress_integration_is_on' => lrm_setting('integrations/bp/on'),
            'redirect_urls'  => array(
	        ...
	        'reCaptcha' => [
		        'is_active' => (LRM_Pro_Security::_get_security_class() === 'LRM_Pro_reCaptcha'),
		        'is_configured' => LRM_Pro_reCaptcha::is_configured(),
		        'is_invisible' => ('invisible' === lrm_setting('security/recaptcha/type')),
	        ]
        );
    ...
    public function check_captcha( $action ) {
    	...
    	if ( in_array($action, ['login', 'signup']) ) {
		    if ( lrm_setting( 'security/general/use_honeypot' ) && isset( $_POST['login'] ) && ! empty( $_POST['login'] ) ) {
			    wp_send_json_error( array(
				    'message' => __( 'You filled out a form field that was created to stop spammers. Please go back and try again or contact the site administrator if you feel this was in error.', 'ajax-login-and-registration-modal-popup' )
			    ) );
		    }
	    }
