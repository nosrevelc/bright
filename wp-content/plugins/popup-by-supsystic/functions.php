 ...
 */
if(!function_exists('dateToTimestampPps')) {
    function dateToTimestampPps($date) {
        if(empty($a))
            return false;
        $a = explode(PPS_DATE_DL, $date);
...
if(!function_exists('importClassPps')) {
    function importClassPps($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), PPS_CODE) !== false) {
					$classFile = preg_replace('/'. PPS_CODE. '/i', '', $classFile);
				}
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, PPS_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], PPS_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeCreateObjPps')) {
    function toeCreateObjPps($class, $params) {
        $className = toeGetClassNamePps($class);
        $obj = NULL;
 ...
 * @return array
 */
if(!function_exists('prepareParamsPps')) {
	function prepareParamsPps(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodePps($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
