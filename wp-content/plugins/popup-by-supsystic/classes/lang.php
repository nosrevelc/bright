...
class langPps {
	...
	static public function extract($d = array('dir' => '', 'langPps' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langPps']) && !empty($d['langPps']))
			$langDirPath = PPS_LANG_DIR. $d['langPps']. DS;
		else
			$langDirPath = PPS_LANG_DIR. PPS_WPLANG. DS;
