<ul style="padding-left: 35px;">
    <?php if ( $post['is_update_first_name']): ?>
        <li> <?php _e('First Name', 'wp_all_import_plugin'); ?></li>
    <?php endif; ?>
    <?php if ( $post['is_update_last_name']): ?>
        <li> <?php _e('Last Name', 'wp_all_import_plugin'); ?></li>
    <?php endif; ?>
    <?php if ( $post['is_update_role']): ?>
        <li> <?php _e('Role', 'wp_all_import_plugin'); ?></li>
    <?php endif; ?>
    <?php if ( $post['is_update_nickname']): ?>
        <li> <?php _e('Nickname', 'wp_all_import_plugin'); ?></li>
    <?php endif; ?>
    <?php if ( $post['is_update_description']): ?>
        <li> <?php _e('Description', 'wp_all_import_plugin'); ?></li>
    <?php endif; ?>
    <?php if ( $post['is_update_login']): ?>
        <li> <?php _e('Login', 'wp_all_import_plugin'); ?></li>
    <?php endif; ?>
    <?php if ( $post['is_update_password']): ?>
        <li> <?php _e('Password', 'wp_all_import_plugin'); ?></li>
    <?php endif; ?>
    <?php if ( $post['is_update_nicename']): ?>
        <li> <?php _e('Nicename', 'wp_all_import_plugin'); ?></li>
    <?php endif; ?>
    <?php if ( $post['is_update_email']): ?>
        <li> <?php _e('Email', 'wp_all_import_plugin'); ?></li>
    <?php endif; ?>
    <?php if ( $post['is_update_registered']): ?>
        <li> <?php _e('Registered Date', 'wp_all_import_plugin'); ?></li>
    <?php endif; ?>
    <?php if ( $post['is_update_display_name']): ?>
        <li> <?php _e('Display Name', 'wp_all_import_plugin'); ?></li>
    <?php endif; ?>
    <?php if ( $post['is_update_url']): ?>
        <li> <?php _e('URL', 'wp_all_import_plugin'); ?></li>
    <?php endif; ?>
