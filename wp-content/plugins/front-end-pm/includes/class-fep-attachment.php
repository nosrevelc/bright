	...
	function display_attachment( $i ) {
		...
		if ( $attachments = $message->get_attachments() ) {
			...
			foreach ( $attachments as $attachment ){
				...
				echo apply_filters( 'fep_filter_attachment_download_link', '<a href="' . fep_query_url( 'view-download', array( 'fep_id' => $attachment->att_id, 'fep_parent_id' => $attachment->mgs_id ) ) . '" target="_blank" title="' . sprintf( __( 'Download %s', 'front-end-pm' ), esc_attr( $name ) ) . '">' . esc_html( $name ) . '</a>', $attachment->att_id, $attachment );
				
				//echo '<img src="'.wp_mime_type_icon( $attachment->att_mime ).'" style="max-width:100%; height: auto;" />';
				echo '</div>';
			}
			echo '</div>';
		}
