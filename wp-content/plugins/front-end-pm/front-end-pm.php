...
class Front_End_Pm {
	...
	function constants() {
		global $wpdb;
		define( 'FEP_PLUGIN_VERSION', '25.2.3' );
		define( 'FEP_DB_VERSION', '2000' );
		...
		if ( ! defined ('FEP_MESSAGE_TABLE' ) ) {
			define( 'FEP_MESSAGE_TABLE', $wpdb->base_prefix . ''.get_current_blog_id().'_fep_messages' );
		}
		if ( ! defined ('FEP_META_TABLE' ) ) {
			define( 'FEP_META_TABLE', $wpdb->base_prefix .  ''.get_current_blog_id().'_fep_messagemeta' );
		}
		if ( ! defined ('FEP_PARTICIPANT_TABLE' ) ) {
			define( 'FEP_PARTICIPANT_TABLE', $wpdb->base_prefix .  ''.get_current_blog_id().'_fep_participants' );
		}
		if ( ! defined ('FEP_ATTACHMENT_TABLE' ) ) {
			define( 'FEP_ATTACHMENT_TABLE', $wpdb->base_prefix .  ''.get_current_blog_id().'_fep_attachments' );
		}
	}
