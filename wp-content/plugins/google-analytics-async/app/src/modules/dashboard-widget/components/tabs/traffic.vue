			...
			<table class="beehive-table">
				<thead>
					<tr>
						<th
							colspan="3"
							class="beehive-column-country"
						>{{ $i18n.dashboard_widget.labels.top_countries }}</th>
					...
					<tr v-for="( item, key ) in stats.countries" :key="key">
						<td colspan="3" class="beehive-column-country">
							<div class="beehive-country-content">
								<span :class="flagClass(item[1])" aria-hidden="true"></span>
								<p class="beehive-country-name">{{ item[0] }}</p>
								<div class="beehive-country-percent" aria-hidden="true">
									<span :style="percentageStyle(item[2])"></span>
								</div>
							</div>
						</td>
				...
				<fragment v-for="( item, key ) in summary" :key="key">
					<div class="beehive-col">
						<div class="beehive-box">
							<h3 class="beehive-box-title" v-html="getBoxTitle( item )"></h3>
							<div class="beehive-box-content">
								<p class="beehive-box-stat-value">{{ getStatValue( key, '%' ) }}</p>
								<p class="beehive-box-stat-name">{{ getStatName( key ) }}</p>
								<sui-score :value="parseInt( getStatValue( key, '' ) )" />
							</div>
						</div>
					</div>
					<span class="beehive-separator" aria-hidden="true" v-if="'social_networks' !== key" />
				</fragment>
			</div>
		</fragment>
	</div>
...
export default {
	...
	data() {
		return {
			summary: {
				...
				mediums: {
					title: this.$i18n.dashboard_widget.labels.top_medium,
					icon: 'update'
				},
				social_networks: {
					title: this.$i18n.dashboard_widget.labels
						.top_social_network,
	...
	methods: {
		...
		getStatName(key) {
			let value = this.$i18n.dashboard_widget.labels.none
