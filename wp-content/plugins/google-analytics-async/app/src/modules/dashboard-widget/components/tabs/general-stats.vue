		...
		<div class="beehive-buttons-wrapper">
			<fragment v-for="( item, key ) in summarySections" :key="key">
				<div class="beehive-button-holder">
					...
					>
						<span class="beehive-button-name">{{ item.title }}</span>
						<span class="beehive-button-value">-</span>
						<i class="beehive-button-icon sui-icon-chevron-right sui-lg" aria-hidden="true"></i>
					</button>

					<button class="beehive-button" @click="openTab( item.tab, key )" v-else>
						<span class="beehive-button-name">{{ item.title }}</span>
						<span class="beehive-button-stats">
							<span class="beehive-button-value beehive-blue beehive-lg">{{ getValue( key ) }}</span>
							<span
			...
			<fragment v-for="( item, key ) in topSections" :key="key">
				<div class="beehive-button-holder">
					...
					<button class="beehive-button" @click="openTab( item.tab )" v-else>
						<span class="beehive-button-name">{{ item.title }}</span>
						<span class="beehive-button-value beehive-blue">{{ getValue( key ) }}</span>
						<i class="beehive-button-icon sui-icon-chevron-right sui-lg" aria-hidden="true"></i>
					</button>
				</div>
			</fragment>
...
export default {
	...
	data() {
		return {
			...
			topSections: {
				page: {
					title: this.$i18n.dashboard_widget.labels.top_page,
					tab: 'top_pages'
				},
				country: {
					title: this.$i18n.dashboard_widget.labels.top_country,
					tab: 'traffic'
				},
				medium: {
					title: this.$i18n.dashboard_widget.labels.top_referral,
					tab: 'traffic'
				},
				search_engine: {
					title: this.$i18n.dashboard_widget.labels.top_search_engine,
					tab: 'traffic'
				}
			}
		}
