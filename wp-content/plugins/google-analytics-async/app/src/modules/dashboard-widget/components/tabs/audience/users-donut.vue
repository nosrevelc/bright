<template>
	<div class="beehive-users-resume">
		...
		<div class="beehive-users-stats">
			<p class="beehive-visitors-empty">{{ $i18n.dashboard_widget.descriptions.no_donut_data }}</p>
			<p class="beehive-visitors-old">
				<span>{{ newVisitors }}%</span>
				{{ $i18n.dashboard_widget.descriptions.new_visitors }}
			</p>
			<p class="beehive-visitors-new">
				<span>{{ returnVisitors }}%</span>
				{{ $i18n.dashboard_widget.descriptions.returning_visitors }}
			</p>
		</div>
	</div>
</template>
		...
		setupChartData() {
			if (this.isEmpty) {
				...
				this.chartData = {
					datasets: [
						{
							label: this.$i18n.dashboard_widget.labels.sessions,
					...
					labels: [
						this.$i18n.dashboard_widget.labels.new_visitors,
						this.$i18n.dashboard_widget.labels.returning_visitors
					]
				}
			}
		}
