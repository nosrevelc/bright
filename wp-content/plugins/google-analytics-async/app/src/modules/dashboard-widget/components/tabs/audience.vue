				...
				<div role="tablist" class="beehive-tabs-menu-wrapper">
					...
					>
						<span class="beehive-item-wrapper">
							<span class="beehive-item-title">{{ item.title }}</span>
							<span class="beehive-item-value" v-if="!isLoggedIn || isEmpty">0</span>
							<span class="beehive-item-stats" v-else>
								<span class="beehive-item-value">{{ getValue(key) }}</span>

								<span
									class="beehive-item-trend beehive-red"
									v-if="getTrendValue(key) > 0 && 'bounce_rates' === key"
								>
									<i class="sui-icon-arrow-up sui-sm" aria-hidden="true"></i>
									{{ Math.abs( getTrendValue(key) ) }}%
								</span>
								<span class="beehive-item-trend beehive-green" v-else-if="getTrendValue(key) > 0">
									<i class="sui-icon-arrow-up sui-sm" aria-hidden="true"></i>
									{{ Math.abs( getTrendValue(key) ) }}%
								</span>
								<span
									class="beehive-item-trend beehive-green"
									v-else-if="getTrendValue(key) < 0 && 'bounce_rates' === key"
								>
									<i class="sui-icon-arrow-down sui-sm" aria-hidden="true"></i>
									{{ Math.abs( getTrendValue(key) ) }}%
								</span>
								<span class="beehive-item-trend beehive-red" v-else-if="getTrendValue(key) < 0">
									<i class="sui-icon-arrow-down sui-sm" aria-hidden="true"></i>
									{{ Math.abs( getTrendValue(key) ) }}%
								</span>
								<span class="beehive-item-trend" v-else>0%</span>
							</span>
						</span>
					</button>
				</div>
	...
	data() {
		return {
			...
			sections: {
				sessions: {
					color: '#17A8E3',
					title: this.$i18n.dashboard_widget.labels.sessions
				},
				users: {
					color: '#2D8CE2',
					title: this.$i18n.dashboard_widget.labels.users
				},
				pageviews: {
					color: '#8D00B1',
					title: this.$i18n.dashboard_widget.labels.pageviews
				},
				page_sessions: {
					color: '#3DB8C2',
					title: this.$i18n.dashboard_widget.labels.page_sessions
				},
				average_sessions: {
					color: '#2B7BA1',
					title: this.$i18n.dashboard_widget.labels.average_sessions
				},
				bounce_rates: {
					color: '#FFB17C',
					title: this.$i18n.dashboard_widget.labels.bounce_rates
				}
			},
