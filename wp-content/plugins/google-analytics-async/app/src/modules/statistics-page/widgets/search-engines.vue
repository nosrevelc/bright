<template>
	<sui-box
		:title="$i18n.statistics.labels.search_engines"
		titleIcon="magnifying-glass-search"
		aria-live="polite"
		:loading="loading"
	>
		<template v-slot:body>
			...
			<fragment v-else>
				<pie-chart class="beehive-chart-pie" :chart-data="chartData" :options="chartOptions" />
				<ul class="beehive-chart-pie-legend">
					<li class="beehive-legend-item" v-for="( item, name ) in stats.search_engines" :key="name">
						<span class="beehive-legend-item-color" aria-hidden="true"></span>
...
export default {
	...
	methods: {
		setupChartData() {
			...
			Object.keys(vm.stats.search_engines).forEach(function(key) {
				chartLabels.push(vm.stats.search_engines[key][0])
				currentData.push(vm.stats.search_engines[key][1])
