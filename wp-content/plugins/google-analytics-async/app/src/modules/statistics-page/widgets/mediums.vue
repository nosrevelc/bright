<template>
	<sui-box
		:title="$i18n.statistics.labels.mediums"
		titleIcon="community-people"
		aria-live="polite"
		:loading="loading"
	>
		<template v-slot:body>
			...
			<fragment v-else>
				...
				<ul class="beehive-chart-pie-legend">
					<li class="beehive-legend-item" v-for="( item, name ) in stats.mediums" :key="name">
...
export default {
	...
	computed: {
		...
		isEmpty() {
			return Object.keys(this.stats).length <= 0 || !this.stats.mediums
		}
	},
	...
	methods: {
		setupChartData() {
			...
			Object.keys(vm.stats.mediums).forEach(function(key) {
				chartLabels.push(vm.stats.mediums[key][0])
				currentData.push(vm.stats.mediums[key][1])
