<template>
	<sui-box
		:title="$i18n.statistics.labels.top_pages"
	...
	>
		<template v-slot:body>
			...
			<table class="beehive-table-pages" v-else>
				<thead>
					<tr>
						<th colspan="2">{{ $i18n.statistics.labels.top_pages_most_visited }}</th>
						<th>{{ $i18n.statistics.labels.average_sessions }}</th>
						<th>{{ $i18n.statistics.labels.views }}</th>
						<th>{{ $i18n.statistics.labels.trend }}</th>
					</tr>
				</thead>
				<tbody>
