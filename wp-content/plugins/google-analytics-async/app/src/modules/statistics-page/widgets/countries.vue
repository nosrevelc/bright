<template>
	<sui-box
		:title="$i18n.statistics.labels.top_countries"
	...
	>
		<template v-slot:body>
			...
			>{{ $i18n.statistics.labels.no_information }}</p>
			...
			<fragment v-else>
				...
				<table class="beehive-chart-map-legend">
					<thead>
						<tr>
							<th>{{ $i18n.statistics.labels.country_code }}</th>
							<th>{{ $i18n.statistics.labels.country_name }}</th>
							<th>{{ $i18n.statistics.labels.visits_percentage }}</th>
							<th>{{ $i18n.statistics.labels.total_visits }}</th>
						</tr>
					</thead>

					<tbody>
						<tr class="beehive-legend-item" v-for="(item, name) in getCountries" :key="name">
							<td class="beehive-legend-item-flag">
								<span class="sui-screen-reader-text">{{ item[1] }}</span>
								<span :class="'beehive-flag beehive-flag-' + item[1]" aria-hidden="true"></span>
							</td>

							<td class="beehive-legend-item-name">{{ item[0] }}</td>

							<td class="beehive-legend-item-bar">
								<span
									class="sui-screen-reader-text"
								>{{ sprintf( $i18n.statistics.descriptions.percentage_visits, visitsPercent( item[2] ) + '%' ) }}</span>
								<span aria-hidden="true">
									<span :style="'width: ' + visitsPercent( item[2] ) + '%;'"></span>
								</span>
							</td>

							<td class="beehive-legend-item-value">{{ item[2] }}</td>
						</tr>
					</tbody>
				</table>
			</fragment>
...
export default {
	...
	data() {
		return {
			...
			chartOptions: {
				chart: {
					title: this.$i18n.statistics.labels.country_sessions
				},
				colorAxis: {
	...
	methods: {
		...
		setupList() {
			...
			dataTable.addColumn('string', this.$i18n.statistics.labels.country)
			dataTable.addColumn('number', this.$i18n.statistics.labels.sessions)
			dataTable.addColumn({
				type: 'string',
				role: 'tooltip',
				p: { html: true }
		...
		geoToolTip(countryName, countryCode, sessions) {
			return (
				...
				this.$i18n.statistics.labels.has +
				'</span> ' +
				'<strong>' +
				sessions +
				'</strong> ' +
				this.$i18n.statistics.labels.sessions +
				'</span>' +
				'</span>'
			)
		},
