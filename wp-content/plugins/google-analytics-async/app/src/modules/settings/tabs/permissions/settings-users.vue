<template>
	<div class="sui-side-tabs sui-tabs">
		<div data-tabs>
			<div class="active">{{ $i18n.settings.buttons.exclude }}</div>
			<div>{{ $i18n.settings.buttons.include }}</div>
		</div>

		<div data-panes>
			<div class="active">
				<p class="sui-description">{{ $i18n.settings.descriptions.exclude_users }}</p>
				<label class="sui-label" v-if="excludeUsers.length">{{ $i18n.settings.labels.excluded_users }}</label>
				...
				>
					...
					{{ $i18n.settings.buttons.add_user }}
				</button>
			</div>
			<div>
				<p class="sui-description">{{ $i18n.settings.descriptions.include_users }}</p>
				<label class="sui-label" v-if="includeUsers.length">{{ $i18n.settings.labels.include_users }}</label>
	...
	methods: {
		...
		removeIncludedUser(id) {
			if (
				...
				this.includeUsers = this.includeUsers.filter(user => user != id)
			}
		},
		...
		removeExcludedUser(id) {
			if (
				...
				this.excludeUsers = this.excludeUsers.filter(user => user != id)
			}
		},
