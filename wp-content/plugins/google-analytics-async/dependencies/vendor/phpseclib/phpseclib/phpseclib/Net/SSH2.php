 ...
 */
namespace Beehive\phpseclib\Net;

use Beehive\phpseclib\Crypt\Base;
use Beehive\phpseclib\Crypt\Blowfish;
use Beehive\phpseclib\Crypt\Hash;
use Beehive\phpseclib\Crypt\Random;
use Beehive\phpseclib\Crypt\RC4;
use Beehive\phpseclib\Crypt\Rijndael;
use Beehive\phpseclib\Crypt\RSA;
use Beehive\phpseclib\Crypt\TripleDES;
use Beehive\phpseclib\Crypt\Twofish;
use Beehive\phpseclib\Math\BigInteger;
// Used to do Diffie-Hellman key exchange and DSA/RSA signature verification.
use Beehive\phpseclib\System\SSH\Agent;
/**
 * Pure-PHP implementation of SSHv2.
 *
 * @package SSH2
...
class SSH2
{
    ...
    function __construct($host, $port = 22, $timeout = 10)
    {
        ...
        $this->channel_open_failure_reasons = array(1 => 'NET_SSH2_OPEN_ADMINISTRATIVELY_PROHIBITED');
        $this->terminal_modes = array(0 => 'NET_SSH2_TTY_OP_END');
        $this->channel_extended_data_type_codes = array(1 => 'NET_SSH2_EXTENDED_DATA_STDERR');
        $this->_define_array(
            ...
            array(60 => 'NET_SSH2_MSG_USERAUTH_INFO_REQUEST', 61 => 'NET_SSH2_MSG_USERAUTH_INFO_RESPONSE'),
            // RFC 4419 - diffie-hellman-group-exchange-sha{1,256}
            array(30 => 'NET_SSH2_MSG_KEXDH_GEX_REQUEST_OLD', 31 => 'NET_SSH2_MSG_KEXDH_GEX_GROUP', 32 => 'NET_SSH2_MSG_KEXDH_GEX_INIT', 33 => 'NET_SSH2_MSG_KEXDH_GEX_REPLY', 34 => 'NET_SSH2_MSG_KEXDH_GEX_REQUEST'),
    ...
    function _key_exchange($kexinit_payload_server = \false)
    {
        ...
        $kex_algorithms = isset($preferred['kex']) ? $preferred['kex'] : $this->getSupportedKEXAlgorithms();
        $server_host_key_algorithms = isset($preferred['hostkey']) ? $preferred['hostkey'] : $this->getSupportedHostKeyAlgorithms();
        $s2c_encryption_algorithms = isset($preferred['server_to_client']['crypt']) ? $preferred['server_to_client']['crypt'] : $this->getSupportedEncryptionAlgorithms();
        $c2s_encryption_algorithms = isset($preferred['client_to_server']['crypt']) ? $preferred['client_to_server']['crypt'] : $this->getSupportedEncryptionAlgorithms();
        ...
        $c2s_compression_algorithms = isset($preferred['client_to_server']['comp']) ? $preferred['client_to_server']['comp'] : $this->getSupportedCompressionAlgorithms();
        // some SSH servers have buggy implementations of some of the above algorithms
        switch (\true) {
            case $this->server_identifier == 'SSH-2.0-SSHD':
            case \substr($this->server_identifier, 0, 13) == 'SSH-2.0-DLINK':
                if (!isset($preferred['server_to_client']['mac'])) {
                    $s2c_mac_algorithms = \array_values(\array_diff($s2c_mac_algorithms, array('hmac-sha1-96', 'hmac-md5-96')));
                }
                if (!isset($preferred['client_to_server']['mac'])) {
                    $c2s_mac_algorithms = \array_values(\array_diff($c2s_mac_algorithms, array('hmac-sha1-96', 'hmac-md5-96')));
                }
        }
        ...
        $client_cookie = \Beehive\phpseclib\Crypt\Random::string(16);
        ...
        if ($kex_algorithm === 'curve25519-sha256@libssh.org') {
            $x = \Beehive\phpseclib\Crypt\Random::string(32);
            $eBytes = \sodium_crypto_box_publickey_from_secretkey($x);
            $clientKexInitMessage = 'NET_SSH2_MSG_KEX_ECDH_INIT';
            $serverKexReplyMessage = 'NET_SSH2_MSG_KEX_ECDH_REPLY';
            $kexHash = new \Beehive\phpseclib\Crypt\Hash('sha256');
        } else {
            if (\strpos($kex_algorithm, 'diffie-hellman-group-exchange') === 0) {
                ...
                $prime = new \Beehive\phpseclib\Math\BigInteger($primeBytes, -256);
                if (\strlen($response) < 4) {
                    return \false;
                }
                \extract(\unpack('NgLength', $this->_string_shift($response, 4)));
                $gBytes = $this->_string_shift($response, $gLength);
                $g = new \Beehive\phpseclib\Math\BigInteger($gBytes, -256);
                ...
                $g = new \Beehive\phpseclib\Math\BigInteger(2);
                $prime = new \Beehive\phpseclib\Math\BigInteger($prime, 16);
            ...
            switch ($kex_algorithm) {
                case 'diffie-hellman-group-exchange-sha256':
                    ...
                    break;
                default:
                    $kexHash = new \Beehive\phpseclib\Crypt\Hash('sha1');
            }
            ...
            $one = new \Beehive\phpseclib\Math\BigInteger(1);
        ...
        if ($kex_algorithm === 'curve25519-sha256@libssh.org') {
            ...
            $key = new \Beehive\phpseclib\Math\BigInteger(\sodium_crypto_scalarmult($x, $fBytes), 256);
            ...
            $f = new \Beehive\phpseclib\Math\BigInteger($fBytes, -256);
        ...
        switch ($mac_algorithm) {
            case 'hmac-sha2-256':
                $this->hmac_create = new \Beehive\phpseclib\Crypt\Hash('sha256');
                $createKeyLength = 32;
                break;
            case 'hmac-sha1':
                $this->hmac_create = new \Beehive\phpseclib\Crypt\Hash('sha1');
                $createKeyLength = 20;
                break;
            case 'hmac-sha1-96':
                $this->hmac_create = new \Beehive\phpseclib\Crypt\Hash('sha1-96');
                $createKeyLength = 20;
                break;
            case 'hmac-md5':
                $this->hmac_create = new \Beehive\phpseclib\Crypt\Hash('md5');
                $createKeyLength = 16;
                break;
            case 'hmac-md5-96':
                $this->hmac_create = new \Beehive\phpseclib\Crypt\Hash('md5-96');
                $createKeyLength = 16;
        }
        ...
        switch ($mac_algorithm) {
            case 'hmac-sha2-256':
                $this->hmac_check = new \Beehive\phpseclib\Crypt\Hash('sha256');
                $checkKeyLength = 32;
                $this->hmac_size = 32;
                break;
            case 'hmac-sha1':
                $this->hmac_check = new \Beehive\phpseclib\Crypt\Hash('sha1');
                $checkKeyLength = 20;
                $this->hmac_size = 20;
                break;
            case 'hmac-sha1-96':
                $this->hmac_check = new \Beehive\phpseclib\Crypt\Hash('sha1-96');
                $checkKeyLength = 20;
                $this->hmac_size = 12;
                break;
            case 'hmac-md5':
                $this->hmac_check = new \Beehive\phpseclib\Crypt\Hash('md5');
                $checkKeyLength = 16;
                $this->hmac_size = 16;
                break;
            case 'hmac-md5-96':
                $this->hmac_check = new \Beehive\phpseclib\Crypt\Hash('md5-96');
                $checkKeyLength = 16;
                $this->hmac_size = 12;
        }
    ...
    function _encryption_algorithm_to_crypt_instance($algorithm)
    {
        switch ($algorithm) {
            case '3des-cbc':
                return new \Beehive\phpseclib\Crypt\TripleDES();
            case '3des-ctr':
                return new \Beehive\phpseclib\Crypt\TripleDES(\Beehive\phpseclib\Crypt\Base::MODE_CTR);
            case 'aes256-cbc':
            case 'aes192-cbc':
            case 'aes128-cbc':
                return new \Beehive\phpseclib\Crypt\Rijndael();
            case 'aes256-ctr':
            case 'aes192-ctr':
            case 'aes128-ctr':
                return new \Beehive\phpseclib\Crypt\Rijndael(\Beehive\phpseclib\Crypt\Base::MODE_CTR);
            case 'blowfish-cbc':
                return new \Beehive\phpseclib\Crypt\Blowfish();
            case 'blowfish-ctr':
                return new \Beehive\phpseclib\Crypt\Blowfish(\Beehive\phpseclib\Crypt\Base::MODE_CTR);
            case 'twofish128-cbc':
            case 'twofish192-cbc':
            case 'twofish256-cbc':
            case 'twofish-cbc':
                return new \Beehive\phpseclib\Crypt\Twofish();
            case 'twofish128-ctr':
            case 'twofish192-ctr':
            case 'twofish256-ctr':
                return new \Beehive\phpseclib\Crypt\Twofish(\Beehive\phpseclib\Crypt\Base::MODE_CTR);
            case 'arcfour':
            case 'arcfour128':
            case 'arcfour256':
                return new \Beehive\phpseclib\Crypt\RC4();
        }
        return null;
    }
    /**
    ...
    function _login_helper($username, $password = null)
    {
        ...
        if ($password instanceof \Beehive\phpseclib\Crypt\RSA) {
            return $this->_privatekey_login($username, $password);
        } elseif ($password instanceof \Beehive\phpseclib\System\SSH\Agent) {
    ...
    {
        $packet = \pack('CNa*Na*Na*Na*Na*', NET_SSH2_MSG_USERAUTH_REQUEST, \strlen($username), $username, \strlen('ssh-connection'), 'ssh-connection', \strlen('keyboard-interactive'), 'keyboard-interactive', 0, '', 0, '');
        if (!$this->_send_binary_packet($packet)) {
            return \false;
        }
    ...
    function _privatekey_login($username, $privatekey)
    {
        ...
        $publickey = $privatekey->getPublicKey(\Beehive\phpseclib\Crypt\RSA::PUBLIC_FORMAT_RAW);
        if ($publickey === \false) {
            return \false;
        }
        ...
        $privatekey->setSignatureMode(\Beehive\phpseclib\Crypt\RSA::SIGNATURE_PKCS1);
            ...
            switch (\true) {
                ...
                default:
                    if (\is_callable($callback)) {
                        if (\call_user_func($callback, $temp) === \true) {
    ...
    function _initShell()
    {
        if ($this->in_request_pty_exec === \true) {
            return \true;
        }
        $this->window_size_server_to_client[self::CHANNEL_SHELL] = $this->window_size;
        $packet_size = 0x4000;
        $packet = \pack('CNa*N3', NET_SSH2_MSG_CHANNEL_OPEN, \strlen('session'), 'session', self::CHANNEL_SHELL, $this->window_size_server_to_client[self::CHANNEL_SHELL], $packet_size);
        if (!$this->_send_binary_packet($packet)) {
            return \false;
        }
        ...
        $terminal_modes = \pack('C', NET_SSH2_TTY_OP_END);
        $packet = \pack('CNNa*CNa*N5a*', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_SHELL], \strlen('pty-req'), 'pty-req', 1, \strlen('vt100'), 'vt100', $this->windowColumns, $this->windowRows, 0, 0, \strlen($terminal_modes), $terminal_modes);
        if (!$this->_send_binary_packet($packet)) {
            return \false;
        }
        $response = $this->_get_binary_packet();
        ...
        $packet = \pack('CNNa*C', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_SHELL], \strlen('shell'), 'shell', 1);
        if (!$this->_send_binary_packet($packet)) {
            return \false;
        }
    ...
    function startSubsystem($subsystem)
    {
        ...
        $packet = \pack('CNa*N3', NET_SSH2_MSG_CHANNEL_OPEN, \strlen('session'), 'session', self::CHANNEL_SUBSYSTEM, $this->window_size, 0x4000);
        if (!$this->_send_binary_packet($packet)) {
            return \false;
        }
        ...
        $packet = \pack('CNNa*CNa*', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_SUBSYSTEM], \strlen('subsystem'), 'subsystem', 1, \strlen($subsystem), $subsystem);
        if (!$this->_send_binary_packet($packet)) {
            return \false;
        }
    ...
    function ping()
    {
        ...
        $packet_size = 0x4000;
        $packet = \pack('CNa*N3', NET_SSH2_MSG_CHANNEL_OPEN, \strlen('session'), 'session', self::CHANNEL_KEEP_ALIVE, $this->window_size_server_to_client[self::CHANNEL_KEEP_ALIVE], $packet_size);
        if (!@$this->_send_binary_packet($packet)) {
            return $this->_reconnect();
        }
    ...
    function _get_binary_packet($skip_channel_filter = \false)
    {
        ...
        if ($remaining_length < -$this->decrypt_block_size || $remaining_length > 0x9000 || $remaining_length % $this->decrypt_block_size != 0) {
            if (!$this->bad_key_size_fix && $this->_bad_algorithm_candidate($this->decrypt->name) && !($this->bitmap & \Beehive\phpseclib\Net\SSH2::MASK_LOGIN)) {
        ...
        if ($this->bitmap & self::MASK_CONNECTED && $this->isAuthenticated()) {
            switch (\ord($payload[0])) {
                ...
                case NET_SSH2_MSG_CHANNEL_WINDOW_ADJUST:
                    ...
                    $payload = $this->bitmap & self::MASK_WINDOW_ADJUST ? \true : $this->_get_binary_packet($skip_channel_filter);
            }
        }
        return $payload;
    }
    ...
    function _get_channel_packet($client_channel, $skip_extended = \false)
    {
        ...
        while (\true) {
            if ($this->binary_packet_buffer !== \false) {
                ...
                $response = $this->_get_binary_packet(\true);
                if ($response === \false) {
                    $this->bitmap = 0;
                    \user_error('Connection closed by server');
                    return \false;
            ...
            if (isset($channel) && isset($this->channel_status[$channel]) && isset($this->window_size_server_to_client[$channel])) {
                ...
                switch ($this->channel_status[$channel]) {
                    case NET_SSH2_MSG_CHANNEL_OPEN:
                        switch ($type) {
                            case NET_SSH2_MSG_CHANNEL_OPEN_CONFIRMATION:
                                ...
                                $result = $client_channel == $channel ? \true : $this->_get_channel_packet($client_channel, $skip_extended);
                    ...
                    case NET_SSH2_MSG_CHANNEL_CLOSE:
                        return $type == NET_SSH2_MSG_CHANNEL_CLOSE ? \true : $this->_get_channel_packet($client_channel, $skip_extended);
                }
            }
    ...
    function _send_binary_packet($data, $logged = null)
    {
        ...
        $padding = \Beehive\phpseclib\Crypt\Random::string($padding_length);
    ...
    function getSupportedEncryptionAlgorithms()
    {
        $algos = array(
            ...
            'aes128-ctr',
            // RECOMMENDED       AES (Rijndael) in SDCTR mode, with 128-bit key
            'aes192-ctr',
            // RECOMMENDED       AES with 192-bit key
            'aes256-ctr',
            // RECOMMENDED       AES with 256-bit key
            'twofish128-ctr',
            // OPTIONAL          Twofish in SDCTR mode, with 128-bit key
            'twofish192-ctr',
            // OPTIONAL          Twofish with 192-bit key
            'twofish256-ctr',
            // OPTIONAL          Twofish with 256-bit key
            'aes128-cbc',
            // RECOMMENDED       AES with a 128-bit key
            'aes192-cbc',
            // OPTIONAL          AES with a 192-bit key
            'aes256-cbc',
            // OPTIONAL          AES in CBC mode, with a 256-bit key
            'twofish128-cbc',
            // OPTIONAL          Twofish with a 128-bit key
            'twofish192-cbc',
            // OPTIONAL          Twofish with a 192-bit key
            'twofish256-cbc',
            'twofish-cbc',
            // OPTIONAL          alias for "twofish256-cbc"
            //                   (this is being retained for historical reasons)
            'blowfish-ctr',
            // OPTIONAL          Blowfish in SDCTR mode
            'blowfish-cbc',
        ...
        $engines = array(\Beehive\phpseclib\Crypt\Base::ENGINE_OPENSSL, \Beehive\phpseclib\Crypt\Base::ENGINE_MCRYPT, \Beehive\phpseclib\Crypt\Base::ENGINE_INTERNAL);
        $ciphers = array();
        foreach ($engines as $engine) {
            foreach ($algos as $algo) {
                $obj = $this->_encryption_algorithm_to_crypt_instance($algo);
                if ($obj instanceof \Beehive\phpseclib\Crypt\Rijndael) {
                    $obj->setKeyLength(\preg_replace('#[^\\d]#', '', $algo));
                }
                switch ($algo) {
                    case 'arcfour128':
                    case 'arcfour256':
                        if ($engine != \Beehive\phpseclib\Crypt\Base::ENGINE_INTERNAL) {
                            continue 2;
                        }
                }
    ...
    function getSupportedMACAlgorithms()
    {
        return array(
            ...
            'hmac-sha1',
            // REQUIRED        HMAC-SHA1 (digest length = key length = 20)
            'hmac-md5-96',
    ...
    {
        $this->_connect();
        return array('kex' => $this->kex_algorithm, 'hostkey' => $this->signature_format, 'client_to_server' => array('crypt' => $this->encrypt->name, 'mac' => $this->hmac_create->name, 'comp' => 'none'), 'server_to_client' => array('crypt' => $this->decrypt->name, 'mac' => $this->hmac_check->name, 'comp' => 'none'));
    }
    /**
    ...
    function setPreferredAlgorithms($methods)
    {
        ...
        if (isset($preferred['kex'])) {
            $preferred['kex'] = \array_intersect($preferred['kex'], $this->getSupportedKEXAlgorithms());
        }
        if (isset($preferred['hostkey'])) {
            $preferred['hostkey'] = \array_intersect($preferred['hostkey'], $this->getSupportedHostKeyAlgorithms());
        }
        ...
        foreach ($keys as $key) {
            if (isset($preferred[$key])) {
                $a =& $preferred[$key];
                if (isset($a['crypt'])) {
                    $a['crypt'] = \array_intersect($a['crypt'], $this->getSupportedEncryptionAlgorithms());
                }
                if (isset($a['comp'])) {
                    $a['comp'] = \array_intersect($a['comp'], $this->getSupportedCompressionAlgorithms());
                }
                if (isset($a['mac'])) {
                    $a['mac'] = \array_intersect($a['mac'], $this->getSupportedMACAlgorithms());
                }
            }
        }
        $keys = array('kex', 'hostkey', 'client_to_server/crypt', 'client_to_server/comp', 'client_to_server/mac', 'server_to_client/crypt', 'server_to_client/comp', 'server_to_client/mac');
        foreach ($keys as $key) {
            $p = $preferred;
            $m = $methods;
            $subkeys = \explode('/', $key);
    ...
    function getServerPublicHostKey()
    {
        ...
        switch ($this->signature_format) {
            case 'ssh-dss':
                $zero = new \Beehive\phpseclib\Math\BigInteger();
                if (\strlen($server_public_host_key) < 4) {
                    return \false;
                }
                $temp = \unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
                $p = new \Beehive\phpseclib\Math\BigInteger($this->_string_shift($server_public_host_key, $temp['length']), -256);
                if (\strlen($server_public_host_key) < 4) {
                    return \false;
                }
                $temp = \unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
                $q = new \Beehive\phpseclib\Math\BigInteger($this->_string_shift($server_public_host_key, $temp['length']), -256);
                if (\strlen($server_public_host_key) < 4) {
                    return \false;
                }
                $temp = \unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
                $g = new \Beehive\phpseclib\Math\BigInteger($this->_string_shift($server_public_host_key, $temp['length']), -256);
                if (\strlen($server_public_host_key) < 4) {
                    return \false;
                }
                $temp = \unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
                $y = new \Beehive\phpseclib\Math\BigInteger($this->_string_shift($server_public_host_key, $temp['length']), -256);
                ...
                $r = new \Beehive\phpseclib\Math\BigInteger($this->_string_shift($signature, 20), 256);
                $s = new \Beehive\phpseclib\Math\BigInteger($this->_string_shift($signature, 20), 256);
                switch (\true) {
                    case $r->equals($zero):
                    case $r->compare($q) >= 0:
                    case $s->equals($zero):
                ...
                $w = $s->modInverse($q);
                $u1 = $w->multiply(new \Beehive\phpseclib\Math\BigInteger(\sha1($this->exchange_hash), 16));
                list(, $u1) = $u1->divide($q);
                $u2 = $w->multiply($r);
                list(, $u2) = $u2->divide($q);
                $g = $g->modPow($u1, $p);
            ...
            case 'rsa-sha2-512':
                ...
                $temp = \unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
                $e = new \Beehive\phpseclib\Math\BigInteger($this->_string_shift($server_public_host_key, $temp['length']), -256);
                if (\strlen($server_public_host_key) < 4) {
                    return \false;
                }
                $temp = \unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
                $rawN = $this->_string_shift($server_public_host_key, $temp['length']);
                $n = new \Beehive\phpseclib\Math\BigInteger($rawN, -256);
                ...
                */
                ...
                $temp = \unpack('Nlength', $this->_string_shift($signature, 4));
                $s = new \Beehive\phpseclib\Math\BigInteger($this->_string_shift($signature, $temp['length']), 256);
                // validate an RSA signature per "8.2 RSASSA-PKCS1-v1_5", "5.2.2 RSAVP1", and "9.1 EMSA-PSS" in the
                // following URL:
                // ftp://ftp.rsasecurity.com/pub/pkcs/pkcs-1/pkcs-1v2-1.pdf
                // also, see SSHRSA.c (rsa2_verifysig) in PuTTy's source.
                if ($s->compare(new \Beehive\phpseclib\Math\BigInteger()) < 0 || $s->compare($n->subtract(new \Beehive\phpseclib\Math\BigInteger(1))) > 0) {
                    \user_error('Invalid signature');
                    return $this->_disconnect(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
                }
                $s = $s->modPow($e, $n);
                ...
                $hashObj = new \Beehive\phpseclib\Crypt\Hash($hash);
                switch ($this->signature_format) {
                    case 'rsa-sha2-512':
                        $h = \pack('N5a*', 0x305130, 0xd060960, 0x86480165, 0x3040203, 0x5000440, $hashObj->hash($this->exchange_hash));
                        break;
                    case 'rsa-sha2-256':
                        $h = \pack('N5a*', 0x303130, 0xd060960, 0x86480165, 0x3040201, 0x5000420, $hashObj->hash($this->exchange_hash));
                        break;
                    //case 'ssh-rsa':
                    default:
                        $hash = 'sha1';
                        $h = \pack('N4a*', 0x302130, 0x906052b, 0xe03021a, 0x5000414, $hashObj->hash($this->exchange_hash));
                }
