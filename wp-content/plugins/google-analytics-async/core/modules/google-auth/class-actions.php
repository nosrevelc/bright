		...
		if ( isset( $state['beehive_nonce'], $state['origin'], $state['default'], $state['page'] ) ) {
			...
			if ( 'network' === $state['origin'] ) {
				...
				if ( 'dashboard' === $state['page'] ) {
					$url = Template::dashboard_page( true );
				} else {
					$url = Template::settings_page( 'tracking', true );
				}
			} else {
				// If from dashboard.
				if ( 'dashboard' === $state['page'] ) {
					$url = Template::dashboard_page();
				} else {
					$url = Template::settings_page( 'tracking', false, $state['origin'] );
				}
			}
		...
		if ( ! empty( $_GET['gcode'] ) ) {
			...
			if ( isset( $token['access_token'], $token['refresh_token'] ) ) {
				...
				if ( ! $network &&
					...
					Helper::instance()->is_logged_in( true ) &&
					'api' === Helper::instance()->login_method( true )
				) {
			...
			// Flag to show notice.
			beehive_analytics()->settings->update( 'google_auth_redirect_success', $success, 'misc', $network );
		}
