...
$singular = __('Point', 'idealbiz');
$plural   = __('Points', 'idealbiz');
...
<div class="ywpar-wrapper">
	<h2><?php echo wp_kses_post( apply_filters( 'ywpar_my_account_my_points_title', sprintf( __( 'My Points', 'idealbiz' ) ) ) ); ?></h2>

	<p>
		<?php
		$points  = sprintf( _nx( '<strong>%1$s</strong> %2$s', '<strong>%3$s</strong> %4$s', ceil($points), 'First placeholder: number of points; second placeholder: label of points;','yith-woocommerce-points-and-rewards' ),  ceil($points), $singular,  ceil($points), $plural );
		...
		$message = __( 'You have %1$s%2$s.', 'yith-woocommerce-points-and-rewards' );
		echo sprintf( wp_kses_post( $message ), wp_kses_post( $points ), wp_kses_post( $worth ) );
		?>
	</p>

	<h3><?php echo wp_kses_post( apply_filters( 'ywpar_my_account_my_points_history_title', sprintf( __( 'My Points History', 'idealbiz' ) ) ) ); ?></h3>
