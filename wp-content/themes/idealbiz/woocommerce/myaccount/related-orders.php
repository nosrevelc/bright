...
<div class="block stroke dropshadow p-30 m-b-25 b-r-5 white--background">
	<h2 class="text-left m-b-15">
		<?php esc_html_e( 'Related Orders', 'woocommerce-subscriptions' ); ?>
	</h2>
	<table class="shop_table shop_table_responsive my_account_orders related_orders m-b-0">
		...
		<tbody>
			<?php foreach ( $subscription_orders as $subscription_order ) {
				...
				?><tr class="order">
					...
					<td class="order-status" data-title="<?php esc_attr_e( 'Status', 'woocommerce-subscriptions' ); ?>">
					...
					<td class="order-actions">
						...
						if ( $actions ) {
							foreach ( $actions as $key => $action ) {
								echo wp_kses_post( '<a href="' . esc_url( $action['url'] ) . '" class="btn btn-blue blue--hover ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>' );
							}
						}
						?>
					</td>
