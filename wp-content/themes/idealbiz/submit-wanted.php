<?php // Template Name: SubmitWantedBusinnes
...
if($did!=''){
    if( current_user_can('editor') || current_user_can('administrator') || get_current_user_id()==get_post_field( 'post_author', $did ) || get_current_user_id() == get_field( 'owner', $did )['ID'] ){
        if ( get_post_type( $did ) == 'wanted' && get_post_status($did)!='trash' ) {
            $p = get_post($did);
            $new_or_edit = $did;
        }
    }
    ...
    <div class="col-md-2">
		...
		<div class="col-md-8">
                <h1 class="text-left m-b-15">
                    <?php _e('Wanted Businness Details', 'idealbiz'); ?>
                </h1>
                <?php
                global $post;
                $post = $p;
                ...
                function my_acf_render_experts( $field ) { ?>
                                     <div class="sidebar-wanted-submission m-t-0">
                                        <div id="contact-this-seller" class="form-content">
                                            ...
                                            <div class="acf-field acf-field-message acf-field-5e60b63f4306c2 h2 m-b-5 w-100"
                                                    ...
                                                    <div class="acf-label m-b-36">
                                                        <h2 style="font-size: 30px!important;color: #14307b;margin-top: 30px;"><?php esc_html_e('Choose your expert', 'idealbiz'); ?></h2>
                                                    </div>
                                                    <div class="acf-notice acf-error-message m-t-0 notice-expert-select">
                                                        <p>
                                                            <?php esc_html_e('If you choose the IDB / Idealbiz Expert, we will assign you the best expert to help you with your Wanted Business', 'idealbiz'); ?>
                                                        </p>
                                                    </div>
                                            ...
                                            <?php //experts
                                                ...
                                                if ($myposts) {
                                                    ...
                                                    foreach ($myposts as $post) {
                                                     ...
                                                     //   $p .= '<div class="swiper-slide p-5" style="height:auto;"><div style="top: 10px;align-items: center;display: flex;justify-content: center;" data-expert="' . $post->ID . '" class="'.$act_class.' p-20 m-b-20 ' . $classes . ' ' . $location_as_classes . ' expert-card position-relative flex-column black--color white--background dropshadow font-weight-medium  w-100"  data-locations="' . join(',', wp_list_pluck($location_objs, 'slug')) . '">';
                ...
                add_action('acf/render_field/name=publish_in', 'my_acf_render_experts');
                    ...
                    $fields = array(
                       ...
                       // 'gallery',
                        //'galeria',
                        //'cover_video',
                        'let_our_staff_create_your_ad_w',
                        'business_type',
                        'category',
                        'location_business',
                        'location',
                    ...
                    $returnurl = get_permalink( wc_get_page_id( 'shop' )).'/?ptype=wanted&id=%post_id%';
                    if(isset($_GET['edit'])){
                        $returnurl = wc_get_endpoint_url('mylistings', '', get_permalink(get_option('woocommerce_myaccount_page_id')));
                    }
                ...
                <div style="display:none;" id="returnurl" data-href="<?php echo wc_get_endpoint_url('mylistings', '', get_permalink(get_option('woocommerce_myaccount_page_id'))); ?>"></div>
                <?php
                    ...
                    jQuery(document).ready(($) => {
                      ...
                      <?php if(LISTING_SYSTEM=='1'){ ?>
                            ...
                            $('select[name="owner"]').on('change', function() {
                                ...
                                <?php if(LISTING_SYSTEM=='1'){ ?>
                                    var aux_system = $('input[name="post_title"]').val('<?php the_title(); ?>') +'|' + $('select[name="owner"] option:selected').text();
                                    $('input[name="listingsystem"]').val();
                                <?php } ?>
                            });
                        <?php } ?>
                    });
